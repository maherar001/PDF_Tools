...
const WordToPdf = () => {
  ...
  const handleFile = (selectedFile: File) => {
    if (
      ...
      alert(t('word_to_pdf.error_file_type'));
      return;
    }
    if (selectedFile.size > 100 * 1024 * 1024) {
      // 100MB limit
      alert(t('word_to_pdf.error_file_size'));
      return;
    }
  ...
  const startConversion = async () => {
    if (!file) {
      alert(t('word_to_pdf.error_no_file'));
      return;
    }
    ...
    try {
      ...
      alert(t('common.conversion_error')); // Add a translation key for this
      setIsConverting(false);
      setConversionComplete(false);
    }
  };
  ...
  return (
    ...
    >
      <div className=''>
        ...
        {/* Conversion Options - Simplified for Word to PDF */}
        <div className='mb-6'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            ...
            <div className='option-group p-4 border border-gray-200 dark:border-gray-700 rounded-lg'>
              ...
              >
                {t('tools.word_to_pdf.output_quality')}
              </label>
              <select
                id='pdf-quality'
            ...
            {/* Add more Word to PDF specific options here if needed */}
          </div>
        </div>
        ...
        {!conversionComplete && !isConverting && file && (
          ...
          >
            ...
            {t('tools.word_to_pdf.convert_button')}
          </button>
        )}
        ...
        {conversionComplete && (
          <div className='mt-6 p-4 bg-green-50 dark:bg-green-900 rounded-lg text-center'>
            ...
            <p className='text-gray-600 dark:text-gray-400 mb-4'>
              {t('tools.word_to_pdf.download_prompt')}
            </p>
            {pdfUrl && (
              <div className='mt-4'>
                <a
                ...
                >
                  {t('tools.word_to_pdf.download_button')}
                </a>
              </div>
            )}
          </div>
